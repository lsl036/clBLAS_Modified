...
#ifndef KERNEL_DGEMM_COL_NN_B0_ML048_NX048_KX08_SRC_H
#define KERNEL_DGEMM_COL_NN_B0_ML048_NX048_KX08_SRC_H

const unsigned int dgemm_Col_NN_B0_ML048_NX048_KX08_workGroupNumRows = 8;
const unsigned int dgemm_Col_NN_B0_ML048_NX048_KX08_workGroupNumCols = 8;
const unsigned int dgemm_Col_NN_B0_ML048_NX048_KX08_microTileNumRows = 6;
const unsigned int dgemm_Col_NN_B0_ML048_NX048_KX08_microTileNumCols = 6;
const unsigned int dgemm_Col_NN_B0_ML048_NX048_KX08_unroll = 8;

const char * const dgemm_Col_NN_B0_ML048_NX048_KX08_src ="\n"
"/* dgemm_Col_NN_B0_ML048_NX048_KX08 */\n"
"\n"
"#pragma OPENCL EXTENSION cl_khr_fp64 : enable\n"
"\n"
"/* kernel parameters */\n"
"#define WG_NUM_ROWS          8\n"
"#define WG_NUM_COLS          8\n"
...
"__kernel void dgemm_Col_NN_B0_ML048_NX048_KX08(\n"
...
"    lA[ 3*localAStride ] = ( globalARow(3) >= M) ? 0.0 : A[ GET_GLOBAL_INDEX_A( globalARow(3), globalACol(3) ) ];\n"
"    lA[ 4*localAStride ] = ( globalARow(4) >= M) ? 0.0 : A[ GET_GLOBAL_INDEX_A( globalARow(4), globalACol(4) ) ];\n"
"    lA[ 5*localAStride ] = ( globalARow(5) >= M) ? 0.0 : A[ GET_GLOBAL_INDEX_A( globalARow(5), globalACol(5) ) ];\n"
...
"    lB[ 3*localBStride ] = B[ GET_GLOBAL_INDEX_B( globalBRow(3), globalBCol(3) ) ];\n"
"    lB[ 4*localBStride ] = B[ GET_GLOBAL_INDEX_B( globalBRow(4), globalBCol(4) ) ];\n"
"    lB[ 5*localBStride ] = B[ GET_GLOBAL_INDEX_B( globalBRow(5), globalBCol(5) ) ];\n"
"    barrier(CLK_LOCAL_MEM_FENCE);\n"
"    uint offA = localRow;\n"
"    uint offB = localCol;\n"
"\n"
...
"  if (globalCRow+0*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+0*WG_NUM_ROWS, globalCCol+3*WG_NUM_COLS) ], alpha, rC[0][3], beta )}\n"
"  if (globalCRow+0*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+0*WG_NUM_ROWS, globalCCol+4*WG_NUM_COLS) ], alpha, rC[0][4], beta )}\n"
"  if (globalCRow+0*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+0*WG_NUM_ROWS, globalCCol+5*WG_NUM_COLS) ], alpha, rC[0][5], beta )}\n"
...
"  if (globalCRow+1*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+1*WG_NUM_ROWS, globalCCol+3*WG_NUM_COLS) ], alpha, rC[1][3], beta )}\n"
"  if (globalCRow+1*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+1*WG_NUM_ROWS, globalCCol+4*WG_NUM_COLS) ], alpha, rC[1][4], beta )}\n"
"  if (globalCRow+1*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+1*WG_NUM_ROWS, globalCCol+5*WG_NUM_COLS) ], alpha, rC[1][5], beta )}\n"
...
"  if (globalCRow+2*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+2*WG_NUM_ROWS, globalCCol+3*WG_NUM_COLS) ], alpha, rC[2][3], beta )}\n"
"  if (globalCRow+2*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+2*WG_NUM_ROWS, globalCCol+4*WG_NUM_COLS) ], alpha, rC[2][4], beta )}\n"
"  if (globalCRow+2*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+2*WG_NUM_ROWS, globalCCol+5*WG_NUM_COLS) ], alpha, rC[2][5], beta )}\n"
"  if (globalCRow+3*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+3*WG_NUM_ROWS, globalCCol+0*WG_NUM_COLS) ], alpha, rC[3][0], beta )}\n"
"  if (globalCRow+3*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+3*WG_NUM_ROWS, globalCCol+1*WG_NUM_COLS) ], alpha, rC[3][1], beta )}\n"
"  if (globalCRow+3*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+3*WG_NUM_ROWS, globalCCol+2*WG_NUM_COLS) ], alpha, rC[3][2], beta )}\n"
"  if (globalCRow+3*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+3*WG_NUM_ROWS, globalCCol+3*WG_NUM_COLS) ], alpha, rC[3][3], beta )}\n"
"  if (globalCRow+3*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+3*WG_NUM_ROWS, globalCCol+4*WG_NUM_COLS) ], alpha, rC[3][4], beta )}\n"
"  if (globalCRow+3*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+3*WG_NUM_ROWS, globalCCol+5*WG_NUM_COLS) ], alpha, rC[3][5], beta )}\n"
"  if (globalCRow+4*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+4*WG_NUM_ROWS, globalCCol+0*WG_NUM_COLS) ], alpha, rC[4][0], beta )}\n"
"  if (globalCRow+4*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+4*WG_NUM_ROWS, globalCCol+1*WG_NUM_COLS) ], alpha, rC[4][1], beta )}\n"
"  if (globalCRow+4*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+4*WG_NUM_ROWS, globalCCol+2*WG_NUM_COLS) ], alpha, rC[4][2], beta )}\n"
"  if (globalCRow+4*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+4*WG_NUM_ROWS, globalCCol+3*WG_NUM_COLS) ], alpha, rC[4][3], beta )}\n"
"  if (globalCRow+4*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+4*WG_NUM_ROWS, globalCCol+4*WG_NUM_COLS) ], alpha, rC[4][4], beta )}\n"
"  if (globalCRow+4*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+4*WG_NUM_ROWS, globalCCol+5*WG_NUM_COLS) ], alpha, rC[4][5], beta )}\n"
"  if (globalCRow+5*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+5*WG_NUM_ROWS, globalCCol+0*WG_NUM_COLS) ], alpha, rC[5][0], beta )}\n"
"  if (globalCRow+5*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+5*WG_NUM_ROWS, globalCCol+1*WG_NUM_COLS) ], alpha, rC[5][1], beta )}\n"
"  if (globalCRow+5*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+5*WG_NUM_ROWS, globalCCol+2*WG_NUM_COLS) ], alpha, rC[5][2], beta )}\n"
"  if (globalCRow+5*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+5*WG_NUM_ROWS, globalCCol+3*WG_NUM_COLS) ], alpha, rC[5][3], beta )}\n"
"  if (globalCRow+5*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+5*WG_NUM_ROWS, globalCCol+4*WG_NUM_COLS) ], alpha, rC[5][4], beta )}\n"
"  if (globalCRow+5*WG_NUM_ROWS < M){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+5*WG_NUM_ROWS, globalCCol+5*WG_NUM_COLS) ], alpha, rC[5][5], beta )}\n"
"\n"
"}\n"
"";
